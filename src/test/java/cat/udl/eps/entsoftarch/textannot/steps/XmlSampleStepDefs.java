...
public class XmlSampleStepDefs {
    ...
    @Autowired
    private XmlSampleRepository xmlSampleRepository;
    @Autowired
    ...
    public void iUploadTheXMLFileWithFilename(String filename, String template) throws Throwable {
        ...
        MockMultipartFile file = new MockMultipartFile("upload", filename, "multipart/form-data", fileInputStream);


        String message = stepDefs.mapper.writeValueAsString(sample);

        stepDefs.result = stepDefs.mockMvc.perform(
                multipart("/upload/xmlsample")
                        .file(file)
                        .param("project", project.getUri())
                        .with(AuthenticationStepDefs.authenticate()))
                .andDo(print());
        newResourceUri = stepDefs.result.andReturn().getResponse().getHeader("Location");
    }
