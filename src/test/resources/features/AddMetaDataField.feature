Feature: Add MetaDataField
  Add field correctly As an admin.
  I want to add a new metadafield.

  Scenario: Create a new metadataField as an admin
    Given I login as "admin" with password "password"
    When I create a new metadatafield with text "newfield" and type "STRING"
    Then The response code is 201
    And It has been created a new metadatafield with text "newfield" and type "STRING" and Id 1


  Scenario: Try to register new metadataField without authenticating
    Given I'm not logged in
    When I create a new metadatafield with text "noauthenticating" and type "STRING"
    Then The response code is 401
    And It has not been created a metadatafield with text "noauthenticating" and type "STRING" and Id 1

  Scenario: Try to register new metadataField as a linguistic
    Given I login as "user" with password "password"
    When I create a new metadatafield with text "noauthenticating" and type "STRING"
    Then The response code is 403
    And It has not been created a metadatafield with text "noauthenticating" and type "STRING" and Id 1

  Scenario: Register new metadataField for existing Project
    Given I login as "admin" with password "password"
    And there is a created Project with name "test_project"
    When I register a new metadataField with text "test_field" and type "STRING" for Project with name "test_project"
    Then The response code is 201
    And It has been created a new metadataField with text "test_field" for Project with name "test_template"