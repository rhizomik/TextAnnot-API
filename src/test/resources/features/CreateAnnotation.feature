Feature: Create Annotation
  ...
  Scenario: Create a new annotation as a linguist
    ...
    And Exists a TagHierarchy with name "tagHierarchy"
    And Exists a Tag with name "tagName" associated to the TagHierarchy "tagHierarchy"
    And Exists a Sample with text "abcdef" and TagHierarchy "tagHierarchy"
  ...
  Scenario: Change the tag of an annotation as a linguist
    Given I login as "user" with password "password"
    And Exists a Tag with name "tagName" associated to the TagHierarchy "tagHierarchy"
    And Exists a Tag with name "tagName2" associated to the TagHierarchy "tagHierarchy"
    And Exists a Sample with text "abcdef" and TagHierarchy "tagHierarchy"
